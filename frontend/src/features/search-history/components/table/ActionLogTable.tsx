...
export function ActionLogTable() {
  ...
  return (
    <>
      <div className="mb-4 mt-5 flex items-center justify-between rounded-xl border border-slate-700/80 bg-slate-900/30 px-5 py-3">
        <p className="text-[13px] font-bold text-slate-400">
          조치 이력은 <span className="text-blue-300">오류코드 검색 결과 화면</span>에서 등록합니다.
        </p>
        <button
          type="button"
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-700/80 bg-slate-950/25">
        ...
        {isLoading ? (
          ...
          <p className="px-6 py-10 text-center text-[14px] font-bold text-slate-500">조치 이력이 없습니다.</p>
        ) : (
          <div className="divide-y divide-slate-800/90">
            {pagedItems.map((item) => {
              ...
              return (
                <article key={item.actionLogId} className="grid gap-3 px-5 py-5 transition hover:bg-blue-500/[0.04] lg:grid-cols-[180px_1fr_1.6fr_120px_140px] lg:items-center lg:px-6">
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-mono text-[16px] font-black text-white">{item.query}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-[14px] font-semibold text-slate-300">{item.duration != null ? `${item.duration}분` : '-'}</p>
                  </div>
