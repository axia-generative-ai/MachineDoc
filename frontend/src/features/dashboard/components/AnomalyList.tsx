...
import type { DashboardAnomalyDigest } from '../infra/dashboard.api';
          ...
          {items.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={`grid h-10 w-[64px] place-items-center rounded-md text-[17px] font-black ${palette.badge}`}>
                  {item.level}
                </span>
                <span className="whitespace-nowrap text-[15px] font-medium text-slate-400">{item.meta}</span>
                <span className="whitespace-nowrap pr-[10px] text-right text-[15px] font-medium text-slate-400">
